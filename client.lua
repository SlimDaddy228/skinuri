...
function SkinCreator(enable)
	...
	if enable == true then
		...
		SetCamCoord(cam, x+0.0, y+2.3, z+0.3)
...
local locations = {
	{-1995.3979492188,3198.6794433594,32.81029510498},
}
...
Citizen.CreateThread(function()
	while true do
		...
		for i,v in pairs(locations) do
			if Vdist(v[1],v[2],v[3],playerPos.x,playerPos.y,playerPos.z) < 1.5 then
				DrawMissionText2("Нажмите ~g~E~w~ чтобы создать персонажа~w~",10)
			...
			if Vdist(v[1],v[2],v[3],playerPos.x,playerPos.y,playerPos.z) <= 12.0 then
				DrawMarker(27,v[1], v[2], v[3]-0.9,0, 0, 0, 0, 0, 0, 1.001,1.001,1.001,255,255,255,200,0,0,0,true)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	for i,v in pairs(locations) do
		...
		AddTextComponentString("Закрытая комната")
...
local locations1 = {
	{-1993.3504638672,3190.80859375,32.810287475586},
}
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		local playerPos = GetEntityCoords(GetPlayerPed(-1), true)
		for i,v in pairs(locations1) do
			if Vdist(v[1],v[2],v[3],playerPos.x,playerPos.y,playerPos.z) < 1.5 then
				DrawMissionText2("Нажмите ~g~E~w~ чтобы покинуть кураторскую~w~",10)
				if IsControlPressed(0,51) then
					teleport(1005.553527832,-2903.244140625,5.9006085395812)
				end
			end
			if Vdist(v[1],v[2],v[3],playerPos.x,playerPos.y,playerPos.z) <= 12.0 then
				DrawMarker(27,v[1], v[2], v[3]-0.9,0, 0, 0, 0, 0, 0, 1.001,1.001,1.001,255,255,255,200,0,0,0,true)
...
function teleport(x,y,z)
	...
	SetEntityCoords(p, x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
